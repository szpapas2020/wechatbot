...
// 初始化机器人 - 使用 Android ADB 模式
const bot = WechatyBuilder.build({
  name: 'wechat-forwarder-bot-adb',
  puppet: 'wechaty-puppet-android',
  puppetOptions: {
    // ADB 连接配置
    adbSerial: process.env.ADB_SERIAL || undefined, // 指定设备序列号（多设备时需要）
  },
});
...
bot.start()
  .then(() => {
    console.log('🤖 微信消息转发机器人已启动 (Android ADB 模式)');
    console.log(`📥 源群: ${config.source.name}`);
    console.log(`📤 目标群: ${config.target.name}`);
    console.log('\n⚠️  请确保已连接 Android 设备并开启 USB 调试\n');
  })
  .catch((error) => {
